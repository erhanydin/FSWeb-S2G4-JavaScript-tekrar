...
function CemberinAlani(radius, piSayisi) {
	let cemberAlani = piSayisi * Math.pow(radius, 2);
	return cemberAlani;
}
...
besyuzdenkucuksayilar = new Array();
sayilar.filter(sayi => {
	if(sayi < 500){
		return besyuzdenkucuksayilar.push(sayi);
	}
})
...
siralisayilar = new Array();

siralisayilar = besyuzdenkucuksayilar.sort(function(sayi1, sayi2){
	return sayi1 - sayi2;
});
...
let tekrarEden = new Object();
 
for(let i = 0; i < sayilar.length; i++){
	if(typeof tekrarEden[sayilar[i]] == 'undefined'){
		tekrarEden[sayilar[i]] = 1;
	} else {
		tekrarEden[sayilar[i]]++;
	}
}

for (let key in tekrarEden) {
	if(tekrarEden[key] > 1){
		tekraredensayilar.push(`${key} sayısı ${tekrarEden[key]} kere tekrar edilmiştir`);
	}
}
